...
if (read.in.raw.maps) {
  ...
  west.boreal.union.buff = gBuffer(west.boreal, width=0.00000) # corrects for non-adjacent boundaries
  boreal.west = intersect(west.boreal.union.buff, boreal.can) # ERROR MESSAGE BELOW
  ##
  ##  Error in RGEOSBinTopoFunc(spgeom1, spgeom2, byid, id, drop_not_poly, "rgeos_intersection") : 
  ##    TopologyException: Input geom 1 is invalid: Ring Self-intersection at or
  ##    near point -1297333.62501293 6927174.4999799998 at -1297333.62501293 6927174.4999799998
  ##
...
if (rasterize.maps) {
  ...
  writeRaster(bcab.all.boreal.raster.brick, filename=file.path(rdata.path, "mpb_brick.grd"), overwrite=TRUE)
  writeRaster(bcab.all.boreal.raster.stack, filename=file.path(rdata.path, "mpb_stack.grd"), overwrite=TRUE)
...
```{r plot.canada.boreal, fig.height=4, fig.width=7, echo=FALSE, cache=use.cache}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.western.boreal, fig.height=5, fig.width=6, echo=FALSE, cache=use.cache}
if (!exists("WORKSPACE")) source("workspace.maps.R")

loadObjects(c("west.boreal"), rdata.path) # also need "boreal.west" once it works
darkgreen <- col2rgb("darkgreen")/255
col.bor <- rgb(darkgreen[1], darkgreen[2], darkgreen[3], alpha=0.5)
rm(darkgreen)
...
```{r plot.mpb.western.boreal.points, fig.height=5, fig.width=6, echo=FALSE, eval=FALSE}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.mpb.western.boreal.polygons, fig.height=5, fig.width=6, echo=FALSE, eval=FALSE}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.mpb.western.boreal.raster.stack, fig.height=6, fig.width=6, echo=FALSE}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.mpb.western.boreal.time.series, fig.height=6, fig.width=6, echo=FALSE}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
brk.ll = projectRaster(bcab.all.boreal.raster.brick, crs=latlongproj,
                       filename=file.path(rdata.path, "mpb_brick_ll.grd"), overwrite=TRUE)

# replace NAs with zeros
brk.ll = brick(lapply(1:nlayers(brk.ll), function(x) { brk.ll[[x]][is.na(brk.ll[[x]])]<- 0; return(brk.ll[[x]]) }))
brk.ll@title <- "MPB intensity"
writeRaster(brk.ll, filename=file.path(rdata.path, "mpb_brick_ll.grd"), overwrite=TRUE)
