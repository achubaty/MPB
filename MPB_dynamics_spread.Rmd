...
objectiveFunction <- function(params) {
  ...
  Sys.sleep(runif(1, 1, 300)) ## random delay to mitigate database locked errors
  
  projDir <- reproducible::checkPath("~/GitHub/MPB")
  scratchDir <- reproducible::checkPath(file.path("/mnt/scratch/MPB"))
  ...
  sim <- SpaDES.core::simInitAndSpades(times = timesFit, params = paramsFit, modules = modules,
                                       #objects = objects,
                                       #useCache = "overwrite",
                                       loadOrder = unlist(modules))

  SpaDES.core::params(sim)[["mpbRedTopSpread"]][["advectionDir"]] <- params[1]
  SpaDES.core::params(sim)[["mpbRedTopSpread"]][["advectionMag"]] <- params[2]
  SpaDES.core::params(sim)[["mpbRedTopSpread"]][["bgSettlingProp"]] <- params[3]
  SpaDES.core::params(sim)[["mpbRedTopSpread"]][["meanDist"]] <- params[4]
  
  simOut <- SpaDES.core::spades(sim, .plotInitialTime = NA)
  ...
  metric <- (atkAreaData - atkAreaSim)^2 ## TODO: something other than simple sum of squares?
  return(metric)
}
...
cl <- parallel::makeCluster(min(N / 2, 32), ## approx 15 GB each for 2010-2011, so max 32 threads
...
paramsPredict[["mpbRedTopSpread"]][["advectionDir"]] <- 90    ## TODO: value from fit
paramsPredict[["mpbRedTopSpread"]][["advectionMag"]] <- 1000  ## TODO: value from fit
paramsPredict[["mpbRedTopSpread"]][["bgSettlingProp"]] <- 0.1 ## TODO: value from fit
