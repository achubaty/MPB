...
if (isTRUE(download.data)) {
  ...
  ## percent pine layers
  destDir <- file.path("kNN-Species", kNN.dir)
  untar(file.path(kNN.dir, dataFiles[1]),
        ...
        exdir = destDir)
  unzip(file.path(destDir, "NFI_MODIS250m_kNN_Species_Pinu_Ban_v0.zip"),
        exdir = file.path("kNN-Species", kNN.dir))
  unzip(file.path(destDir, "NFI_MODIS250m_kNN_Species_Pinu_Con_v0.zip"), exdir = kNN.dir)
  
  ## age & total volume layers
  destDir <- file.path("kNN-StructureStandVolume", kNN.dir)
  untar(file.path(kNN.dir, dataFiles[2]),
        ...
        exdir = outDir)
  unzip(file.path(destDir, "NFI_MODIS250m_kNN_Structure_Stand_Age_v0.zip"), exdir = destDir)
  unzip(file.path(destDir, "NFI_MODIS250m_kNN_Structure_Volume_Total_v0.zip"), exdir = destDir)
}
```
...
kNN_percentPine_map <- mosaic(kNN_percentPine[[1]], kNN_percentPine[[2]], fun = sum) %>% 
  setNames("Lodgepole_and_Jack_Pine")

writeRaster(kNN_percentPine_map, filename = file.path(kNN.dir, "kNN_percentPine_map.tif"))
save(kNN_percentPine_map, file = file.path(kNN.dir, "kNN_percentPine_map.Rdata"))
```
...
kNN_volumePine <- kNN_percentPine * kNN_volumeTotal / 100
