...
#    `functionName` is a function's name in your module (e.g., `canDEMEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/canDEM/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    canDEM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("canDEMEvent1", envir = .GlobalEnv)) {
    simOutput <- canDEMEvent1(mySim)
  } else {
    simOutput <- mySim$canDEMEvent1(mySim)
  }
  ...
  if (exists("canDEMEvent2", envir = .GlobalEnv)) {
    simOutput <- canDEMEvent2(mySim)
  } else {
    simOutput <- mySim$canDEMEvent2(mySim)
  }
