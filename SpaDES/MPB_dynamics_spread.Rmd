...
The second is the linearized multi-niche model of @Berryman:1999bk, which, oddly & inexplicably, held endemic-niche dynamics as a fast process (`R[t] ~ X[t-1]`), and epidemic-niche dynamics as a lagged process (`R[t] ~ X[t-2]`) (`Berryman1979_forced`). [NOT YET IMPLEMENTED]

**We also implement Boone / MacQuarrie stuff**
...
Optimization approaches, specifically Pattern Oriented Modeling (POM; *e.g.*, Grimm 2005) are used to select the dispersal kernel and its parameters, as well as the parameters associated with asymmetric spreading.

### `canWindBiosim`

Import/use wind projections from BioSim to be used with `mpbSpread`.
...
Install the most recent version of the `SpaDES` package from the development branch (packages version `1.3.1.9067` or higher is required.)
...
if (!require(amc) || packageVersion("amc") < "0.1.1.9000") {
  devtools::install_github("achubaty/amc@development", dependencies = TRUE)
}
if (!require(SpaDES) || packageVersion("SpaDES") < "1.3.1.9067") {
