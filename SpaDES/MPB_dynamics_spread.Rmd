...
Consequently, the module offers the option of using any of four possible interpretations (B79fit, B79forced, XXX, YYY).
Two addiitonal options are forthcoming at the time of writing.

The first of these is a high-order polynomial fit to Berryman's hand-drawn curve (`mpbGrowth_B79fit`).
The second is the linearized multi-niche model of @Berryman:1999bk, which, oddly & inexplicably, held endemic-niche dynamics as a fast process (`R[t] ~ X[t-1]`), and epidemic-niche dynamics as a lagged process (`R[t] ~ X[t-2]`) (`mpbGrowth_B79forced`). [IS THIS RIGHT??]
