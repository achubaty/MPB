...
#    `functionName` is a function's name in your module (e.g., `mpbRedAttackEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/mpbRedAttack/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("mpbRedAttack")
  ...
  parameters <- list(
    ...
    mpbRedAttack = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mpbRedAttackEvent1", envir = .GlobalEnv)) {
    simOutput <- mpbRedAttackEvent1(mySim)
  } else {
    simOutput <- mySim$mpbRedAttackEvent1(mySim)
  }
  ...
  if (exists("mpbRedAttackEvent2", envir = .GlobalEnv)) {
    simOutput <- mpbRedAttackEvent2(mySim)
  } else {
    simOutput <- mySim$mpbRedAttackEvent2(mySim)
  }
