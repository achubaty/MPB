...
#    `functionName` is a function's name in your module (e.g., `mpbManagementEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/mpbManagement/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("mpbManagement")
  ...
  parameters <- list(
    ...
    mpbManagement = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mpbManagementEvent1", envir = .GlobalEnv)) {
    simOutput <- mpbManagementEvent1(mySim)
  } else {
    simOutput <- mySim$mpbManagementEvent1(mySim)
  }
  ...
  if (exists("mpbManagementEvent2", envir = .GlobalEnv)) {
    simOutput <- mpbManagementEvent2(mySim)
  } else {
    simOutput <- mySim$mpbManagementEvent2(mySim)
  }
