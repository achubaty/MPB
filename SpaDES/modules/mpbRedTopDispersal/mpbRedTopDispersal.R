...
doEvent.mpbRedTopDispersal <- function(sim, eventTime, eventType, debug = FALSE) {
  switch(eventType,
    "init" = {
      ...
      sim <- sim$mpbRedTopDispersalInit(sim)
  
      # schedule future event(s)
      sim <- scheduleEvent(sim, time(sim) + params(sim)$mpbRedTopDispersal$dispersalInterval,
                           "mpbRedTopDispersal", "dispersal")
      sim <- scheduleEvent(sim, params(sim)$mpbRedTopDispersal$.plotInitialTime, "mpbRedTopDispersal", "plot")
      sim <- scheduleEvent(sim, params(sim)$mpbRedTopDispersal$.saveInitialTime, "mpbRedTopDispersal", "save")
    },
    "dispersal" = {
      # ! ----- EDIT BELOW ----- ! #
      # do stuff for this event
  
      sim <- sim$mpbRedTopDispersalDispersal(sim)
    ...
    "plot" = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "mpbRedTopDispersal", "plot")
...
mpbRedTopDispersalInit <- function(sim) {
...
mpbRedTopDispersalPlot <- function(sim) {
  # ! ----- EDIT BELOW ----- ! #
  # do stuff for this event
  #Plot("object")
...
mpbRedTopDispersalDispersal <- function(sim) {
  # ! ----- EDIT BELOW ----- ! #
